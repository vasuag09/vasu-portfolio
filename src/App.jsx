...
const Portfolio = () => {
  ...
  const careerTrajectory = [
    {
      ...
      desc: "Python, NumPy, Pandas, Basic Regression",
      level: 25,
    },
    {
      year: "2022",
      title: "Deep Learning & CNNs",
      desc: "TensorFlow, PyTorch, Computer Vision (Medical)",
      level: 55,
    },
    {
      year: "2023",
      title: "Full-Stack AI",
      desc: "React, FastAPI, Cloud Deployment, System Design",
      level: 80,
    },
    {
      year: "2024",
      title: "LLMs & Agents",
      desc: "RAG, LangChain, Vector DBs, Latency Optimization",
      level: 98,
    },
  ];
  ...
  const projects = [
    {
      ...
      description:
        "End-to-end monocular traffic intelligence system that detects, tracks, counts vehicles, and estimates relative speed from real-world highway video using event-based motion analysis.",
      link: "https://github.com/vasuag09/intelligent-traffic-analytics-system", // replace with GitHub or demo video link
      status: "LIVE",
      gif: null,
      tier: "S",
      details: {
        problem:
          "Urban traffic monitoring using monocular CCTV feeds is challenging due to unstable tracking IDs, perspective distortion, occlusions, and lack of camera calibration.",

        architecture:
          "Single-camera perception pipeline: Video Input → YOLO Vehicle Detection → Kalman Filter-based Multi-Object Tracking → Event-Based Counting Logic → Motion-Based Speed Estimation → Aggregated Traffic Metrics.",

        pipeline:
          "Frame Ingestion → Vehicle Detection (YOLO) → Centroid-Based Tracking → Line-Crossing Event Detection → Velocity Extraction from Kalman State → Traffic Flow & Speed Analytics.",

        decisions:
          "Used event-based counting instead of relying on persistent track IDs to handle ID fragmentation. Estimated speed from Kalman velocity state rather than raw bounding-box displacement to reduce noise.",

        failures:
          "Absolute speed estimation is sensitive to camera placement and perspective distortion. Addressed by reporting relative speed trends and clearly documenting calibration limitations.",

        metrics:
          "Successfully processed real highway footage with stable vehicle counts and relative speed estimation. System maintained real-time performance on single-camera input.",
      },
    },
    {
      id: 2,
      ...
      description:
        "Full-stack React application with AI-enhanced resume recommendations. Engineered a reusable UI system and secure backend for job tracking.",
      link: "https://insightify-react-frontend.onrender.com/",
      status: "LIVE",
      gif: null,
      tier: "S", // System / Flagship
      details: {
        problem:
          "Job seekers struggle to optimize resumes for ATS (Applicant Tracking Systems) without feedback. Manual comparison is subjective and slow.",
        architecture:
          "Decoupled Architecture: React Frontend ↔ Node.js API Gateway ↔ Python NLP Microservice. This separation prevents heavy text processing from blocking the Node.js event loop.",
        pipeline:
          "1. PDF Parsing (PyPDF2) → 2. Text Cleaning (Regex) → 3. Entity Extraction (Spacy NER) → 4. Vector Similarity (TF-IDF/Cosine) → 5. Scoring Logic.",
        decisions:
          "Chose Microservices over Monolith to allow independent scaling of the NLP worker. Used Redux for state management to cache analysis results and reduce API calls.",
        failures:
          "Initial OCR failed on double-column PDF layouts. Solved by implementing heuristic layout analysis before extraction.",
        metrics:
          "Parses standard resumes in <1.2s. 90% accuracy on keyword extraction compared to manual review.",
      },
    },
    {
      id: 3,
      ...
      description:
        "End-to-end medical imaging pipeline. Implemented Grad-CAM for explainability and CLAHE normalization to achieve high accuracy on heterogeneous data sources.",
      link: "https://github.com/vasuag09/Covid-X-Ray",
      status: "RESEARCH",
      gif: null,
      tier: "A", // Core Engineering
      details: {
        problem:
          "Rapid screening of COVID-19 from Chest X-Rays is needed, but deep learning models often learn spurious correlations (e.g., hospital markers) instead of pathology.",
        architecture:
          "Transfer Learning with DenseNet121 backbone. Added custom classification head (GlobalAvgPool → Dense → Dropout → Softmax).",
        pipeline:
          "Input Image → CLAHE Normalization (Contrast Enhancement) → Augmentation (Rotation/Zoom) → CNN Feature Extractor → Classification.",
        decisions:
          "Selected DenseNet121 over ResNet50 for better feature propagation and parameter efficiency. Implemented Grad-CAM to verify the model was actually looking at the lungs.",
        failures:
          "Model initially overfitted on source-specific artifacts. Fixed by rigorous cross-dataset validation and aggressive augmentation.",
        metrics:
          "96% Sensitivity, 94% Specificity on test set. Inference time <200ms on standard GPU.",
      },
    },
    {
      id: 4,
      ...
      description:
        "Retrieval-Augmented Generation system for academic notes. Capable of retrieving context, analyzing documents, and generating accurate citations.",
      link: "#",
      status: "BUILDING",
      gif: null,
      tier: "A", // Core Engineering
      details: {
        problem:
          "Students waste time searching through hundreds of pages of academic PDFs for specific concepts.",
        architecture:
          "RAG Pipeline: PDF Loader → RecursiveCharTextSplitter → OpenAI Embeddings → FAISS Vector Store → LLM Chain.",
        pipeline:
          "Query → Vector Search (Top-K Retrieval) → Context Injection → LLM Generation → Answer.",
        decisions:
          "Used FAISS for local vector storage to minimize latency and cost compared to Pinecone. Implemented 'Sources' return to hallucination checks.",
        failures:
          "Chunking logic initially broke sentences, losing context. Optimized chunk overlap to 200 tokens.",
    ...
    },
    {
      ...
      description:
        "High-precision sentiment analysis on 14k+ tweets. Deployed as an interactive interface for real-time customer feedback classification.",
      ...
      details: {
        problem:
          "Airlines receive thousands of tweets daily. Manual triage is impossible during disruptions.",
        architecture:
          "Streamlit Frontend ↔ Scikit-Learn Inference Pipeline (Pickled Model).",
        pipeline:
          "Text Preprocessing (Stopwords/Stemming) → TF-IDF Vectorization (Unigrams+Bigrams) → Logistic Regression Classifier.",
        decisions:
          "Chose Logistic Regression over BERT for this iteration to prioritize inference speed and interpretability (feature weights).",
        failures:
          "Sarcasm detection remains a challenge with bag-of-words models. Future work involves LSTM/Transformers.",
        metrics:
          "82% Accuracy on multiclass classification. <50ms inference time.",
      },
    },
    {
      id: 6,
      ...
      description:
        "Fraud-classification pipeline with engineered features. Deployed interface for real-time risk scoring of insurance claims.",
      ...
      details: {
        problem:
          "Fraudulent claims cost insurers billions. Detecting patterns in tabular data requires robust feature engineering.",
        architecture:
          "Standard ML Pipeline: Data Ingestion → Preprocessing → Random Forest Classifier.",
        pipeline:
          "SMOTE (Synthetic Minority Over-sampling) → Feature Scaling → Model Training → Threshold Tuning.",
        decisions:
          "Used SMOTE to handle severe class imbalance (fraud cases <1%). Optimized for Recall over Precision to minimize missed fraud cases.",
        failures:
          "High False Positive rate initially. Tuned decision threshold to balance risk/operational load.",
    ...
    },
    {
      ...
      description:
        "Fine-tuned ResNet50 achieving >95% accuracy. Implemented structured evaluation with confusion matrices and class-level performance metrics.",
      ...
      details: {
        problem:
          "Classifying 102 flower categories requires fine-grained visual discrimination.",
        architecture: "ResNet50 (Pretrained on ImageNet) → Fine-tuning layers.",
        pipeline:
          "Resize/CenterCrop → Normalization → ResNet Forward Pass → Softmax.",
        decisions:
          "Utilized 'One Cycle' learning rate policy for faster convergence. Froze early layers to preserve generic feature detectors.",
        failures:
          "Confusion between visually similar species (e.g., two types of lilies).",
        metrics:
          "95.4% Test Accuracy. Training time reduced by 40% via LR scheduling.",
      },
    },
    {
      id: 8,
      ...
      description:
        "Comprehensive pricing model for Rio de Janeiro rentals. Full exploratory data analysis workflow to identify key value drivers in real estate.",
      ...
      details: {
        problem:
          "Hosts struggle to set optimal prices. Underpricing loses revenue; overpricing loses bookings.",
        architecture: "XGBoost Regressor with Grid Search Cross-Validation.",
        pipeline:
          "EDA (Correlation Heatmaps) → Outlier Removal → Feature Encoding → XGBoost Training.",
        decisions:
          "Chose XGBoost for its ability to handle non-linear relationships and feature importance outputs.",
        failures:
          "Location data (Lat/Long) was initially noisy. Clustered locations into neighborhoods for better stability.",
    ...
    },
    {
      ...
      description:
        "Production-grade classification workflow for banking data. Handles class imbalance with calibrated probability outputs for risk assessment.",
      ...
      details: {
        problem:
          "Banks need to identify at-risk customers before they leave to offer retention incentives.",
        architecture: "Ensemble Method (Gradient Boosting + Random Forest).",
        pipeline:
          "Preprocessing → Feature Selection (RFE) → Model Stacking → Probability Calibration.",
        decisions:
          "Focus on 'Probability Calibration' so the bank can prioritize high-value at-risk customers.",
        failures:
          "Demographic bias detected in initial model. Re-weighted samples to ensure fairness.",
    ...
    },
    {
      ...
      description:
        "Interactive handwritten digit recognition app (~99% accuracy). Real-time inference UI deployed on Streamlit for instant testing.",
      ...
      details: {
        problem:
          "Classic computer vision benchmark to demonstrate end-to-end ML deployment.",
        architecture:
          "Conv2D (32) → MaxPool → Conv2D (64) → MaxPool → Flatten → Dense (128) → Output.",
        pipeline:
          "Canvas Input → Grayscale Conversion → Resizing (28x28) → Normalization → Inference.",
        decisions:
          "Used 'Canvas' library in Streamlit to allow real-time drawing input from users.",
        failures:
          "Users drawing in corners or with thin lines caused prediction errors. Added centering preprocessing step.",
    ...
    },
    {
      ...
      description:
        "Foreground segmentation and vehicle detection system using temporal filtering and morphological operations. Optimized for static camera feeds.",
      link: "#",
      status: "CODE",
      gif: null,
      tier: "A", // Core Engineering
      details: {
        problem:
          "Counting vehicles in static CCTV feeds without heavy deep learning compute.",
        architecture: "Classical Computer Vision Pipeline (No Neural Nets).",
        pipeline:
          "Gaussian Mixture Models (GMM) Background Subtraction → Morphological Opening/Closing → Blob Analysis.",
        decisions:
          "Chose GMM over simple frame differencing to handle changing lighting conditions.",
        failures:
          "Shadows were initially detected as vehicles. Tuned thresholding parameters to suppress shadows.",
  ...
  const skills = [
    {
      category: "GEN AI / LLM",
      items: ["LangChain", "RAG Pipelines", "HuggingFace", "FAISS", "LLM"],
    },
    {
      category: "DEEP LEARNING",
      items: ["TensorFlow", "PyTorch", "Computer Vision", "Grad-CAM", "CNN"],
    },
    {
      category: "FULL STACK",
      items: ["React", "Node.js", "MongoDB", "Streamlit", "MERN Stack"],
    },
    {
      category: "DATA ENG",
      items: ["Pandas", "SQL", "Data Pipelines", "Model Debugging", "EDA"],
    },
  ];
  ...
  // ... (Effect hooks, Terminal logic, shortcuts, pipeline animator remain the same) ...
  useEffect(() => {
    ...
    if (e.key === "Enter") {
      ...
      switch (cmd) {
        ...
        case "./open":
          const target = args[1];
          if (target) {
            let proj = !isNaN(parseInt(target))
            ...
            systemOutput = [
              "Error: Missing argument. Usage: ./open insightify",
            ];
          }
          break;
        case "./stats":
          ...
          if (statTarget) {
            ...
            if (proj && proj.details) {
              ...
              systemOutput = [`Error: Project '${statTarget}' not found.`];
            }
          } else {
            systemOutput = [
              "Error: Missing argument. Usage: ./stats covid-xray",
            ];
          }
          break;
        default:
      ...
      if (systemOutput) {
        if (Array.isArray(systemOutput))
          setTerminalHistory((prev) => [
            ...prev,
            ...systemOutput.map((line) => ({ type: "system", content: line })),
          ]);
        else
          setTerminalHistory((prev) => [
            ...prev,
            { type: "system", content: systemOutput },
          ]);
      } else {
  ...
  const NavButton = ({ id, icon: Icon, label, shortcut }) => (
    <button
      ...
      className={`w-full flex items-center justify-between px-4 py-3 text-sm font-mono transition-all duration-200 border-l-2 cursor-pointer ${
        activeTab === id && !expandedProject && !readingLog
          ? "border-emerald-500 bg-emerald-500/10 text-emerald-400"
          : "border-transparent text-slate-400 hover:text-emerald-300 hover:bg-slate-800"
      }`}
    >
      ...
      {shortcut && (
        <span className="text-[10px] opacity-50 border border-slate-700 px-1 rounded hidden lg:block">
          {shortcut}
        </span>
      )}
    </button>
  ...
  // ... (Render components: Pipeline, Tour, Terminal, Deep Dive, Reading Log remain largely the same, mostly styling adjustments if needed for tiers) ...
                ...
                <div className="flex-1 h-[2px] bg-slate-800 mx-2 relative overflow-hidden">
                  <motion.div
                    ...
                    className={`absolute inset-0 bg-gradient-to-r from-transparent via-emerald-500 to-transparent ${
                      isPast ? "opacity-100 w-full bg-emerald-900" : "w-1/2"
                    }`}
                  />
                </div>
              )}
  ...
  const renderTour = () => {
    ...
    const tourContent = [
      {
        title: "Welcome to VASU_OS",
        text: "This is a high-performance engineering portfolio designed for clarity and speed. Let's take a quick look around.",
      },
      {
        title: "Project Deployments",
        text: "Access 10+ deployed AI systems here. Click 'View Analysis' on any card for a deep architectural breakdown.",
      },
      {
        title: "Developer Terminal",
        text: "Power user? Press Cmd+K (or Ctrl+K) anytime to open the CLI. It is now AI-POWERED—ask it anything about me.",
      },
      {
        title: "Engineering Log",
        text: "I don't just write code; I explain it. Read my long-form debugging logs and system design decisions here.",
      },
      {
        title: "Ready to Collaborate?",
        text: "I am open to high-impact roles. Download my CV or initiate communication directly.",
      },
    ];
    return (
      <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 cursor-auto">
        ...
        >
          ...
          <button
            ...
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
    ...
    >
      ...
      >
        ...
        >
          ...
          {isAiProcessing && (
            <div className="pl-5 text-emerald-500/50 animate-pulse">
              Thinking...
            </div>
          )}
  ...
  // RENDER DEEP DIVE VIEW (Updated with Tier Badge in Header)
  if (expandedProject) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className={`min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-emerald-500/30 selection:text-emerald-200 relative ${
          isRetro ? "retro-mode" : ""
        }`}
      >
        {isRetro && (
        ...
        <div className="md:hidden p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 backdrop-blur">
          ...
          <div className="text-xs text-slate-500">MODE: DEEP_DIVE</div>
        </div>
        <div className="max-w-7xl mx-auto min-h-screen flex flex-col">
          ...
          <motion.div
            ...
            className="p-6 md:p-12 grid lg:grid-cols-3 gap-12"
          >
            <div className="lg:col-span-2 space-y-12">
              <div>
                ...
                <p className="text-slate-300 leading-relaxed text-lg border-l-4 border-emerald-500 pl-6">
                  {expandedProject.details?.problem}
                </p>
              </div>
              <div>
                ...
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-lg font-mono text-sm text-slate-400">
                  {expandedProject.details?.architecture}
                </div>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white flex items-center gap-2 mb-4">
                  <GitBranch className="text-emerald-500" /> Engineering
                  Pipeline
                </h3>
                <p className="text-slate-400 leading-relaxed">
                  {expandedProject.details?.pipeline}
                </p>
              </div>
            </div>
            <div className="space-y-8">
              <div className="bg-slate-900/30 border border-slate-800 p-6 rounded-lg">
                ...
                <p className="text-slate-400 text-sm leading-relaxed">
                  {expandedProject.details?.decisions}
                </p>
              </div>
              <div className="bg-red-900/10 border border-red-500/20 p-6 rounded-lg">
                ...
                <p className="text-slate-400 text-sm leading-relaxed">
                  {expandedProject.details?.failures}
                </p>
              </div>
              <div className="bg-emerald-900/10 border border-emerald-500/20 p-6 rounded-lg">
                ...
                <div className="text-slate-300 font-mono text-sm">
                  {expandedProject.details?.metrics}
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="mb-8">
              ...
              <div className="flex items-center gap-4 text-sm text-slate-500 font-mono">
                ...
                <span>•</span>
                <span>{readingLog.readTime} read</span>
              </div>
            </div>
  ...
  // MAIN RENDER (Modified Project Cards to show Tiers)
  return (
    <div
      className={`min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-emerald-500/30 selection:text-emerald-200 relative ${
        isRetro ? "retro-mode" : ""
    ...
    >
      ...
      <AnimatePresence>{tourStep > 0 && renderTour()}</AnimatePresence>
            ...
            >
              <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-900">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <Play size={16} className="text-emerald-500" /> Demo:{" "}
              ...
              <div className="bg-black p-2 flex justify-center items-center min-h-[400px]">
                {previewProject.gif ? (
                  <img
                    src={previewProject.gif}
                    alt={`${previewProject.title} Demo`}
      ...
      <div className="md:hidden p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 backdrop-blur">
        ...
        <div className="text-xs text-slate-500">V 4.2.0</div>
      </div>
      ...
      <div className="flex flex-col md:flex-row max-w-7xl mx-auto min-h-screen">
          ...
          <div>
            ...
            <div className="px-6 mt-6 hidden md:block">
              <button
                ...
                className="w-full bg-slate-900 border border-slate-700 p-2 rounded text-xs font-mono text-slate-400 hover:text-emerald-500 hover:border-emerald-500/50 transition-all flex items-center justify-between group cursor-pointer mb-2"
              >
                <div className="flex items-center gap-2">
                  <Command size={12} /> Terminal
                </div>
                <span className="bg-slate-800 px-1.5 py-0.5 rounded text-[10px] group-hover:text-white">
                  ⌘K
                </span>
              </button>
              <button
                onClick={() => setIsRetro((prev) => !prev)}
                className={`w-full border p-2 rounded text-xs font-mono transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  isRetro
              ...
              <button
                ...
                className="w-full border border-slate-700 p-2 rounded text-xs font-mono text-slate-400 hover:text-emerald-500 hover:border-emerald-500/50 transition-all flex items-center justify-center gap-2 cursor-pointer mt-2 bg-slate-900"
              >
        ...
        <main className="flex-1 md:ml-64 p-6 md:p-12 lg:p-16 overflow-y-auto">
          <header className="flex justify-between items-end mb-12 border-b border-slate-800 pb-4">
            ...
            <div className="flex items-end gap-6">
              <div className="hidden md:block text-right">
                <a
                  href="https://drive.google.com/file/d/1oPs0eZW7bvuWO032v5NZQs-E1PFO2jp9/view?usp=sharing"
                  target="_blank"
                  rel="noreferrer"
          ...
          <AnimatePresence mode="wait">
            {activeTab === "overview" && (
              ...
              >
                <div className="bg-slate-900/50 border border-slate-800 p-8 rounded-lg relative overflow-hidden group">
                  ...
                  <h1 className="text-3xl md:text-5xl font-bold text-white mb-6 max-w-2xl leading-tight">
                    Build fast. <br />
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-500 text-white font-mono px-6 py-3 rounded flex items-center gap-2 transition-all active:scale-95 cursor-pointer"
                    >
                      VIEW SHIPMENTS <ChevronRight size={16} />
                    </button>
                    <a
                      href="https://drive.google.com/file/d/1oPs0eZW7bvuWO032v5NZQs-E1PFO2jp9/view?usp=sharing"
                      target="_blank"
            ...
            {/* Content: PROJECTS (Updated Card with Tiers) */}
            {activeTab === "projects" && (
              <motion.div
                key="projects"
                initial={{ opacity: 0, y: 20 }}
                  ...
                  {filteredProjects.length > 0 ? (
                    filteredProjects.map((project) => (
                      <motion.div
                        ...
                        className="group bg-slate-900/30 border border-slate-800 p-6 rounded-lg hover:border-emerald-500/50 transition-all hover:bg-slate-900/50 flex flex-col relative overflow-hidden"
                      >
                        {/* Tier Badge */}
                        <div className="absolute top-0 right-0 p-2 z-10">
                          {project.tier === "S" && (
                            <div className="flex items-center gap-1 bg-yellow-500/10 border border-yellow-500/50 px-2 py-0.5 rounded text-[10px] font-bold text-yellow-400 shadow-[0_0_10px_rgba(234,179,8,0.2)]">
                              <Sparkles size={10} /> TIER S
                            </div>
                          )}
                          {project.tier === "A" && (
                            <div className="flex items-center gap-1 bg-blue-500/10 border border-blue-500/50 px-2 py-0.5 rounded text-[10px] font-bold text-blue-400">
                              TIER A
                            </div>
                          )}
                          {project.tier === "B" && (
                            <div className="flex items-center gap-1 bg-slate-700/30 border border-slate-600 px-2 py-0.5 rounded text-[10px] font-bold text-slate-400">
                              TIER B
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex justify-between items-start mb-4 gap-4 mt-2">
                          ...
                          <span
                            className={`text-[10px] font-mono px-2 py-1 rounded border flex items-center gap-2 shrink-0 ${
                              project.status === "LIVE"
            ...
            {/* Other tabs (Skills, Blog, About) remain unchanged... */}
            {activeTab === "skills" && (
              <motion.div
                key="skills"
                initial={{ opacity: 0, y: 20 }}
                ...
                <div className="font-mono text-sm space-y-6 text-slate-300">
                  <div className="bg-slate-950 border border-slate-800 p-6 rounded">
                    ...
                    <p className="mb-4 leading-relaxed">
                      I am an AI/ML engineer and full-stack developer focused on
                      building real, deployable systems. I specialize in deep
                      learning, model debugging, computer vision, classical ML
                      pipelines, and building AI-powered web applications.
                    </p>
                    <p className="mb-4 leading-relaxed">
                      I work across the stack—Python, ML, TensorFlow,
                      React/Next.js, Node.js—and prioritize shipping end-to-end
                      projects that demonstrate clear engineering depth and
                      practical value.
                    </p>
